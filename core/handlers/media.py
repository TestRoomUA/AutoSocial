...
from core.keyboards.inline import market_product_keyboard
...
async def send_post(chat: int, bot: Bot, post: Post, markup: InlineKeyboardMarkup | ReplyKeyboardMarkup = None):
    ...
    markup = post_markup(markup, post.page)
...
def post_markup(markup: InlineKeyboardMarkup | ReplyKeyboardMarkup = None, param: int = None):
    if markup is None:
        return market_product_keyboard(param)
    return markup
